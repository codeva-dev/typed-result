...
The repository root is the package root, so a specific GitHub ref can also be installed without waiting for an npm publish:
...
The repository is still a monorepo: the root is the publishable package, and `examples` is a private TypeScript workspace that compiles against the root package.
