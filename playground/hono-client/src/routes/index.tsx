    ...
    <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col gap-6 px-6 py-10">
      <header className="space-y-2">
        <p className="text-sm font-medium uppercase tracking-wide text-slate-500">
          Hono REST API + TanStack Query
        </p>
        <h1 className="text-3xl font-semibold text-slate-950">Todo manager</h1>
        <p className="max-w-2xl text-sm leading-6 text-slate-600">
          The Hono API returns ordinary HTTP responses. This React client maps
          actionable HTTP states into core typed-result values.
        </p>
      </header>
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => {
      Result.handle(result)
        .tapSuccess(() => {
          setTitle('')
          void queryClient.invalidateQueries({ queryKey: todosQueryOptions.queryKey })
        })
        .tapFailure((failure) => {
  ...
  return (
    <MatchFailureTags
      ...
      tags={{
        ...
        TodoRequestFailure: (failure) => <ErrorPanel>{failure.message}</ErrorPanel>,
